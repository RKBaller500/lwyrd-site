...
export default async function EditFirmPage({
  ...
  const initialData: FirmInput = {
    ...
    founded: firm.founded_year ?? firm.founded ?? 0,
    size: firm.size,
    billingModel: firm.primary_billing_model ?? firm.billing_model,
    hourlyRate: firm.hourly_rate ?? null,
    budgetMin: firm.typical_budget_min ?? firm.budget_min ?? 0,
    budgetMax: firm.typical_budget_max ?? firm.budget_max ?? 0,
    responseTime: firm.response_time,
    overallScore: firm.overall_score ?? 0,
    verified: firm.is_verified ?? firm.verified ?? false,
    logoUrl: firm.logo_url ?? null,
    description: firm.bio ?? firm.description ?? "",
