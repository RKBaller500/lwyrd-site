  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#E7E7E3] rounded-3xl overflow-hidden">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {filtered.map((firm, i) => (
            <tr key={firm.id} className={`border-b border-[#E7E7E3] last:border-0 ${i % 2 === 0 ? "" : "bg-white/40"}`}>
              ...
              <td className="px-5 py-3.5">
                <div className="flex flex-wrap gap-1">
                  {(firm.practice_areas ?? []).slice(0, 3).map((area: string) => (
                    <span key={area} className="px-2 py-0.5 bg-[#002B55]/8 text-[#002B55] rounded-full text-xs">{area}</span>
                  ))}
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center gap-3 justify-end">
                  <Link href={`/admin/firms/${firm.id}`} className="text-xs text-[#002B55] hover:opacity-70 transition-opacity font-medium">Edit</Link>
